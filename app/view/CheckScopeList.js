 ...
 * @create 2013-09-26
 */
Ext.define('iPass.view.CheckScopeList', {
	extend : 'Ext.Container',
	xtype : 'checkScopeList',
...
//							'<div class="rowscontent text-overflow" style="color:#759297;">{Content}</div>',
							...
							'<tpl if="this.hasAttachList(AttachList)">',
								'<div class="rowscontent" style="margin:3px auto 0px -1px">',
								    '<div style="position:relative;width:100%;height:40px;margin-left:4px">',
									 	'<tpl for="AttachList">',
									 		'<div style="position: absolute;width:40px;height:40px;{[this.setLeftVal(xindex)]}">',
											  ...
											  '<input type="image" onclick="checkPointActionSheetCtr.viewPic(\'{parent.AttachListString}\',{[xindex]},\'checkScopeList\')" src="{Key}" width="40px" height="40px" />',
										    '</div>',
									    '</tpl>',
									'</div>',
								'</div>',
						...
						'</div>',
						'<tpl if="PMChecked">',
							'<div class="checkPoint-enable" onclick="checkPointListCtr.goToCheckPointListHis(\'{ItemCode}\',\'CheckScopeList\')"></div>',							
						'<tpl else>',
							'<div class="checkPoint-disable" onclick="checkPointListCtr.goToCheckPointListHis(\'{ItemCode}\',\'CheckScopeList\')"></div>',
						'</tpl>',
					'<tpl else>',
